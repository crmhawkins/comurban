...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900">
            Crear Nueva Tool ElevenLabs
        </h1>
        <p class="mt-2 text-gray-600">Configura una nueva herramienta para que el chatbot de ElevenLabs pueda usarla</p>
    </div>

    <!-- Formulario -->
    <div class="bg-white rounded-lg shadow border border-gray-200 p-6">
        <form method="POST" action="{{ route('elevenlabs.tools.store') }}">
            @csrf
            ...
            <div class="mb-6">
                ...
                <select
                    ...
                    class="block w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 cursor-pointer"
                >
                    <option value="whatsapp" {{ old('platform', 'elevenlabs') === 'whatsapp' ? 'selected' : '' }}>WhatsApp</option>
                    <option value="elevenlabs" {{ old('platform', 'elevenlabs') === 'elevenlabs' ? 'selected' : '' }}>ElevenLabs</option>
                    <option value="both" {{ old('platform', 'elevenlabs') === 'both' ? 'selected' : '' }}>Ambas (WhatsApp y ElevenLabs)</option>
                </select>
            ...
            <div class="flex items-center justify-end space-x-4">
                <a
                    href="{{ route('elevenlabs.tools.index') }}"
...
<script>
    ...
    window.loadTemplateVariables = function(templateId) {
        ...
        // Hacer petición AJAX
        fetch(`{{ route('elevenlabs.tools.template-variables') }}?template_id=${templateId}`, {
            headers: {
