...
class ProcessWebhookEvent implements ShouldQueue
{
    ...
    {
        try {
            // Check if there's already an AI response to THIS specific incoming message
            // This prevents duplicate responses if the webhook is processed multiple times
            $existingAIResponse = $conversation->messages()
                ->where('direction', 'outbound')
                ->where('id', '>', $incomingMessage->id)
                ->where(function($query) use ($incomingMessage) {
                    $query->whereJsonContains('metadata->ai_generated', true)
                          ->whereJsonContains('metadata->in_response_to', $incomingMessage->id);
                })
                ->first();

            if ($existingAIResponse) {
                Log::info('Skipping AI response: already responded to this incoming message', [
                    'conversation_id' => $conversation->id,
            ...
            // Check if there's recent human activity in the conversation
            // If a human sent a message recently (not AI), don't respond automatically
            // This prevents AI from interrupting when a human is actively chatting
            $recentOutboundMessages = $conversation->messages()
                ->where('direction', 'outbound')
                ->where('id', '<', $incomingMessage->id)
            ...
            // Find the most recent message that was NOT generated by AI
            $recentHumanMessage = null;
            foreach ($recentOutboundMessages as $msg) {
                $isAIGenerated = isset($msg->metadata['ai_generated']) && $msg->metadata['ai_generated'] === true;
                if (!$isAIGenerated) {
                    $recentHumanMessage = $msg;
                    break;
                }
            }

            if ($recentHumanMessage) {
                // If there's a human message sent in the last 15 minutes, don't respond with AI
                // This indicates a human is actively managing the conversation
                $minutesSinceHumanMessage = $recentHumanMessage->created_at->diffInMinutes(now());
                if ($minutesSinceHumanMessage < 15) {
                    Log::info('Skipping AI response: recent human activity detected in conversation', [
                        'conversation_id' => $conversation->id,
                        'incoming_message_id' => $incomingMessage->id,
                        'last_human_message_id' => $recentHumanMessage->id,
                        'minutes_since_human_message' => $minutesSinceHumanMessage,
                    ]);
                    return;
                }
            }

            // Check if the last message in the conversation is already from AI
            // This prevents sending a new AI response if the last message was already from AI
            // (meaning we already responded and shouldn't respond again to the same conversation state)
            $lastMessage = $conversation->messages()
                ->orderBy('created_at', 'desc')
                ->orderBy('id', 'desc')
                ->first();

            if ($lastMessage && $lastMessage->id !== $incomingMessage->id) {
                ...
                if ($lastMessage->direction === 'outbound' && 
                    ...
                    Log::info('Skipping AI response: last message is from AI and was created very recently', [
                        ...
                        'incoming_message_id' => $incomingMessage->id,
                        'seconds_ago' => $lastMessage->created_at->diffInSeconds(now()),
                    ]);
                    return;
                }
            }
